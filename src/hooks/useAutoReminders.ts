...
import { format, addHours, parseISO, getDaysInMonth } from "date-fns";
...
export const useAutoReminders = ({ students, payments, settings: appSettings }: UseAutoRemindersProps) => {
  ...
  const sendReminder = useCallback(
    async (phone: string, message: string, studentName: string, type: "session" | "payment" | "cancellation") => {
      try {
        // Clean phone number
        let cleanedPhone = phone.replace(/\D/g, "");

        const { data, error } = await supabase.functions.invoke("send-whatsapp-reminder", {
          body: { phone: cleanedPhone, message, studentName, type },
        });
  ...
  const checkSessionReminders = useCallback(async () => {
    ...
    const now = new Date();
    const reminderHours = settings.session_reminder_hours || 24;
  ...
  // Check and send payment reminders (X days before end of month)
  const checkPaymentReminders = useCallback(async () => {
    ...
    for (const student of students) {
      ...
      const monthlyPayment = studentPayment?.payments.find((p) => p.month === currentMonth && p.year === currentYear);
