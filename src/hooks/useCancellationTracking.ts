...
export const useCancellationTracking = (students: Student[]) => {
  ...
  const getCancellationCount = useCallback(
    (studentId: string, month?: string) => {
      ...
      // ✅ Count from actual cancellations array (always accurate)
      const count = cancellations.filter((c) => c.studentId === studentId && c.month === targetMonth).length;
  ...
  const getStudentCancellations = useCallback(
    (studentId: string, month?: string) => {
      const targetMonth = month || getCurrentMonth();
      return cancellations.filter((c) => c.studentId === studentId && c.month === targetMonth);
    },
    [cancellations],
  );
  ...
  const wasParentNotified = useCallback(
    (studentId: string, month?: string) => {
      const targetMonth = month || getCurrentMonth();
      const tracking = monthlyTracking.find((t) => t.studentId === studentId && t.month === targetMonth);
  ...
  const recordCancellation = useCallback(
    async (
      ...
      try {
        ...
        const { error: insertError } = await supabase.from("session_cancellations").insert({
        ...
        const { error: upsertError } = await supabase.from("student_cancellation_tracking").upsert(
          {
            student_id: studentId,
            month,
...
شكراً لتعاونكم`;
      ...
      try {
        ...
        if (error) {
          ...
          await logNotification(studentId, parentPhone, message, triggeredBy, undefined, "failed", error.message);
          return { success: false, error: error.message };
        }

        // Log successful notification
        await logNotification(studentId, parentPhone, message, triggeredBy, data?.messageSid, "sent");
    ...
    return students
      .map((student) => {
        ...
        const severity: "safe" | "warning" | "critical" | "exceeded" =
          percentage >= 100 ? "exceeded" : percentage >= 75 ? "critical" : percentage >= 50 ? "warning" : "safe";
